...
test_dataset = test_dataset.map(lambda image, image_shape: scale_image_with_crop_padding(image,image_shape, args.crop_size))
...
with tf.Session() as sess:
	...
	while True:
		try:
			batch_images_np, batch_predictions_np = \
				sess.run([batch_images_tf, predictions_tf])
			...
			for i in range(batch_predictions_np.shape[0]):
				...
				a,b = input_image.shape[0],input_image.shape[1]
				print(a,b)
				if input_image.shape[0] == 263169:
					pred_image = np.reshape(pred_image, (a,b))
